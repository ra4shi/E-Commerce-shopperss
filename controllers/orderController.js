...
var instance = new Razorpay({
    key_id: 'rzp_test_2gNBJtL41koWDQ',
...
const verifyPayment = async (req,res)=>{
    try{
        ...
        let hmac = crypto.createHmac('sha256', 'TnvXZAMKPdBrMv1JpeXmMtEd');
...
const showOrder = async (req,res)=>{
    try{
        ...
        if(req.session.user_id){
            const customer = true;
            res.render('showorder',{userName,customer,orderData});
        }else{
            res.redirect('/');
        }
    }catch(error){
...
const viewOrderProducts = async(req,res)=>{
    ...
    const userName = await User.findOne({_id:req.session.user_id});
    const orderId = await Order.findOne({_id:id}).populate("products.productId");
    console.log(orderId);
    const products = orderId.products;
    console.log(products+"vieworderproducts");
